...
import { MutualFundsData, MutualFundsCalculationMethod } from '../../types/calculator';
import { formatCurrency } from '../../utils/formatters';

const liabilityConditions = [
  {
    id: 'below_nisab',
    label: 'Below the Nisab Threshold',
    description:
      'Zakat is only obligatory if the total value of the wealth reaches the nisab \u2014 the minimum threshold for zakat. For investment funds, this usually follows the nisab for trade goods or gold: 85 grams of pure gold.',
  },
  {
    id: 'non_zakatable_assets',
    label: 'Asset Types in the Fund Are Not Zakatable',
    description:
      'Not all instruments inside a mutual fund are zakatable assets. Examples: Real estate held not for sale (e.g., for rental income), fixed assets (buildings, machinery) owned by portfolio companies, long-term sukuk with fixed returns after expenses. In such cases, the principal capital is not zakatable, only income/rent/dividends are zakatable.',
  },
];

const calculationMethods: Array<{
  value: MutualFundsCalculationMethod;
  label: string;
  description: string;
}> = [
  {
...
const MutualFundsCalculatorScreen: React.FC = () => {
  ...
  const existingData = useCalculatorStore((state) => state.calculators.mutual_funds);
  ...
  const { control, watch, handleSubmit } = useForm<FormData>({
    defaultValues: {
      calculationMethod: existingData?.calculationMethod || 'ratio_25',
  ...
  const calculationSummary = useMemo(() => {
    ...
    holdings.forEach((holding) => {
      ...
      totalAssets += calculationMethod === 'ratio_25' ? currentValue * 0.25 : currentValue;
    });
  ...
  const onSave = (data: FormData) => {
    const fundsData: MutualFundsData = {
    ...
    setCalculatorData('mutual_funds', fundsData);
    markAsCalculated('mutual_funds');
  };

  const importantNotes = [
    'Must hold mutual fund for one lunar year (355 days).',
    '25% ratio method applies to zakatable portion of fund assets.',
    'Only zakatable assets within the fund are subject to zakat.',
  ];
    ...
    <>
      ...
      <Modal visible={showLiabilityCheck} animationType="slide" presentationStyle="pageSheet">
        <View style={styles.modalContainer}>
          ...
          <Text style={styles.modalSubtitle}>
            Check if any of these conditions apply to your Mutual Funds
          </Text>
          ...
          <View style={styles.modalFooter}>
            <View style={styles.modalFooterButtons}>
              ...
              />
              <Button
                title={isExempt ? 'Continue Anyway' : 'None Apply - Proceed'}
                onPress={() => setShowLiabilityCheck(false)}
                style={styles.modalButton}
              />
            </View>
      ...
      >
          ...
          <View style={styles.exemptBanner}>
            ...
            <View style={styles.exemptTextContainer}>
              ...
              <Text style={styles.exemptDescription}>
                Based on your liability check, no Zakat is due on these mutual funds.
              </Text>
            </View>
        ...
        <View style={styles.formSection}>
          ...
          <Text style={styles.sectionSubtitle}>Add your mutual fund holdings to calculate Zakat</Text>
            ...
            return (
              <View key={field.id} style={styles.holdingCard}>
                <View style={styles.holdingHeader}>
                  <Text style={styles.holdingNumber}>Fund {index + 1}</Text>
                  {fields.length > 1 && (
                    <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  warningBanner: {
    ...
    borderRadius: borderRadius.lg, padding: spacing.lg, marginHorizontal: spacing.lg, marginBottom: spacing.lg,
  },
  ...
  conditionCard: {
    ...
    padding: spacing.lg, marginBottom: spacing.md, borderWidth: 1, borderColor: colors.gray[200], gap: spacing.md,
  },
  ...
  checkbox: {
    ...
    backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center', marginTop: 2,
  },
  ...
  exemptBanner: {
    ...
    borderWidth: 1, borderColor: colors.amber[200], gap: spacing.md,
  },
  ...
  liabilityButton: {
    ...
    padding: spacing.md, backgroundColor: colors.primary[50], borderRadius: borderRadius.lg,
    borderWidth: 1, borderColor: colors.primary[200], marginBottom: spacing.lg,
  },
  ...
  methodCard: { backgroundColor: colors.white, borderRadius: borderRadius.lg, padding: spacing.lg, borderWidth: 1, borderColor: colors.gray[200], marginTop: spacing.sm },
  methodOption: { paddingVertical: spacing.md },
  ...
  methodLabel: { fontSize: typography.fontSizes.base, fontWeight: typography.fontWeights.semibold, color: colors.gray[900], marginBottom: 4 },
  methodLabelSelected: { color: colors.primary[700] },
  methodDescription: { fontSize: typography.fontSizes.sm, color: colors.gray[600], lineHeight: 20 },
  ...
  calculatedField: { marginBottom: spacing.md },
  calculatedLabel: { fontSize: typography.fontSizes.sm, fontWeight: typography.fontWeights.medium, color: colors.gray[700], marginBottom: spacing.sm },
  calculatedValueBox: { backgroundColor: colors.gray[100], borderRadius: borderRadius.md, padding: spacing.md, borderWidth: 1, borderColor: colors.gray[200] },
  calculatedValue: { fontSize: typography.fontSizes.base, fontWeight: typography.fontWeights.semibold, color: colors.gray[900] },
  addButton: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm,
    padding: spacing.md, backgroundColor: colors.primary[50], borderRadius: borderRadius.lg,
...
export default MutualFundsCalculatorScreen;
